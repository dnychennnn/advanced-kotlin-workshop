...
//}
//
////inline fun <A, B> Iterable<A>.map(transformation: (A) -> B): List<B> {
...
////}
//
//inline fun <A, B> Iterable<A>.map(transformation: (A) -> B) {
//    fold(emptyList<B>()){acc, a -> acc + transformation(a) }
//}
//
...
//
//    println(names.map { it.uppercase(Locale.getDefault()) }) // [MIKE, JANE, MARCIN, JOHN, JAMES]
